...
def parse_args():
    ...
    parser.add_argument(
        "--num_warmup_steps", type=int, default=4000, help="Number of steps for the warmup in the lr scheduler."
    )
    ...
    parser.add_argument("--pe_type", type=str, choices=["seq_pe", "sin", "rotary", "vanilla", "nope", "alibi"], default="vanilla", help="Position encoding method.")
    parser.add_argument("--pe_apply_method", type=str, choices=["input_add", "rotary", "rotary_2d", "nope", "rotary_mixed", "attn_mul", "attn_add", "attn_scalar", "alibi"], default="input_add", help="Position encoding method.")
    ...
    parser.add_argument("--use_pe_qk_per_layer", type=str, default=None, choices=['single', 'multi', 'none'], help='whether create weights that map a single PE to query and key PEs at each layer. If false, we will do the qk map only at the begining, rather than each layer.  Note this is only use for pe_apply_method in ["attn_mul", "attn_add", "attn_scalar"].')

    # seqpe args
    parser.add_argument("--seqpe_dist_sample_range", type=int, default=256)
    ...
    parser.add_argument("--seqpe_scalar_scale", type=float, default=1.0, help="scale seqpe scalar attention")
    parser.add_argument("--seqpe_attn_pdrop", type=float, default=0.1)
    parser.add_argument("--seqpe_resid_pdrop", type=float, default=0.1)
    parser.add_argument("--seqpe_decay", type=float, default=0.0)
    ...
    parser.add_argument("--seqpe_contrastive_num", type=int, default=32, help="number of neg examples")
    ...
    if args.use_pe_multi_head:
        if args.pe_apply_method == "input_add" or args.pe_apply_method == "nope" or args.pe_apply_method == "rotary" or args.pe_apply_method == "alibi":
            raise ValueError
        elif args.pe_type == "seq_pe" and not args.seqpe_add_out_proj:
            raise ValueError
    args.use_pe_qk_per_layer = None if args.use_pe_qk_per_layer == 'none' else args.use_pe_qk_per_layer
    if args.use_pe_qk_per_layer is not None:
        assert args.pe_type == "seq_pe" and args.pe_apply_method.startswith("attn")
    #################################

    return args


def build_optimizer(model, pe_model, args):
    # Optimizer
    ...
    if pe_model is not None and args.pe_type == "seq_pe":
...
def build_dataset(args):
    if args.dataset_name is not None:
        ...
        assert "validation" in raw_datasets.keys() and "train" in raw_datasets.keys()
    else:
...
def process_dataset(accelerator, raw_datasets, tokenizer, args):
    ...
    with accelerator.main_process_first():
        ...
        train_dataset = tokenized_datasets["train"].map(
            partial(group_texts, block_size=block_size, stride=block_size, drop_last=True),
            batched=True,
            batch_size=5000,
            num_proc=args.preprocessing_num_workers,
            load_from_cache_file=not args.overwrite_cache,
...
def main():
    ...
    # build pe model
    pe_config = get_pe_config(args)
    model, config, tokenizer = build_model_and_tokenizer(args, pe_config, attn_method=args.attn_method)
    assert args.num_attention_heads == config.num_attention_heads
    pe_model = build_pe_model(pe_config)
    optimizer = build_optimizer(model, pe_model, args)
    ...
    if STUDY_MODE:
        eval_out = validate(accelerator, model, config, eval_dataloader, pe_model, pe_config, pe_main_task_sampler, word_num=eval_word_num)
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(active_dataloader):
            ...
            with accelerator.accumulate(model):
                ...
                if pe_config.PE_TYPE == "seq_pe":
                    if use_pe_ct and use_pe_transfer and cur_pe_main_batch_size == 1:
                    ...
                    elif use_pe_transfer and cur_pe_main_batch_size == 1:
                        pe_trans_inputs = pe_trans_sampler.next(device)
                ...
                if use_pe_transfer and cur_pe_main_batch_size == 1:
            ...
            if completed_steps % checkpointing_steps == 0 and accelerator.sync_gradients:
                ...
                if accelerator.is_main_process:
                    if eval_out['token_ppl'] < best_ckpt['ppl']:
                        logger.info(f"Found best model | step {completed_steps}\ttoken ppl: {eval_out['token_ppl']:.4f}\tword ppl: {eval_out['word_ppl']:.4f}")
                    ...
                    logger.info(
                        ...
                        f"best token ppl {best_ckpt['ppl']}\t"
                    )
                    ...
                    wandb.log(
                        {
                            ...
                            "valid/word_ppl": eval_out['word_ppl'],
