    ...
    def __init__(self, data_shape, max_sample_position, max_digits, pad_value=0, data_dim=1, add_cls_token_pe=False, device=torch.device("cpu"), use_random_shift=False, random_shift_rate=1, random_shift_downsample=320, default_start_pos=None, start_epoch=0):
        ...
        if data_dim == 1:
            ...
            if self.use_random_shift:
                ...
                self.indices = indices[:random_shift_downsample]
...
class SeqPeContrstiveDataSampler:
    ...
    """
    def __init__(self, n_batch_per_epoch, batch_size, max_sample_position, max_digits, contrastive_size, distributional_sample_range=196, pad_value=0, data_dim=1, debug_mode=False, max_prefetch=32, start_epoch=0, seed=None):
        ...
        """
        self.method_ratio = [0.5, 0.8, 1.0]
        self.hard_neg_ratio = 0.2
    ...
    def init_dataset(self, debug_mode=False):
    ...
    def distributional_sample(self, n_sample):
        """
        ...
        """
        if self.data_dim == 1:
            start_point = int(self.max_sample_position * random.uniform(0, 1))
    ...
    def _prepare_batch(self, pivot_list, device=torch.device("cpu"), debug_mode=None):
        ...
        for i in range(batch_size):
            if debug_mode is not None:
                ...
                rand_mode = random.uniform(0, 1)
                mode = (rand_mode > self.method_ratio[0]) + (rand_mode > self.method_ratio[1])
            if mode == 0:
                special_negative_list = []
            elif mode == 1:
                ...
                special_negative_list = self.distributional_sample(contrastive_size)
...
class SeqPeTransferDataSampler:
    ...
    def init_dataset(self,):
        ...
        ds = self.data_shape
        if self.data_dim == 1:
            pos_list = list(range(ds[0] * ds[1], n_max-ds[0] * ds[1]))
        else:
